...
export default {
    ...
    foldNext() {
        console.log('Fold next called');
        var editor = atom.workspace.getActiveTextEditor();
        var screenLineCount = editor.getScreenLineCount();
        var screenLine = editor.getSelectedScreenRange().start.row;
        var isLineFoldable = () => editor.isFoldableAtScreenRow(screenLine);

        while (!isLineFoldable() && screenLine < screenLineCount) {
            screenLine++;
            console.log(screenLine, isLineFoldable());
        }

        var bufferLine = editor.bufferPositionForScreenPosition([screenLine, 0]).row;
        editor.foldBufferRow(bufferLine);

        // TODO: Store the screenLine/bufferLine in some way for repeated executions, but forget it if editor loses focus or something like that. Or move cursor to next line after the folded one. Could probably scroll to folded line too
